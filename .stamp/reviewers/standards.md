...
## Project-specific conventions for dispatch

Read `AGENTS.md` for the full project surface; the items below are the
load-bearing code conventions worth restating.

- **Stack: Bun + strict TypeScript + zod + smol-toml.** Stay native to
  these. Don't import heavy alternatives that duplicate built-ins or the
  existing dependency set. `bun run typecheck` must pass.
- **The `sinks/` pattern.** External write surfaces live in
  `src/sinks/<name>.ts` (currently `vault.ts`, `labels.ts`, `security.ts`).
  New write surfaces follow the same shape; new *read* code goes in
  `github.ts` / `vault-read.ts`.
- **Shell out, don't reimplement.** Wrappers around `gh` and `oteam` go
  in `github.ts` / `sinks/vault.ts` and stay thin. Don't reimplement
  their command surface.
- **Logging is structured JSON via `src/log.ts`.** Never `console.log`
  free-form strings outside `src/cli.ts`.
- **Failures must not poison the tick.** Per-repo and per-issue errors
  get caught in the poll loop; the next repo / next issue continues.
  New code that runs inside `pollOnce` / `processIssue` follows the
  same pattern.
- **Cursors advance on success only.** A failure on issue N leaves the
  cursor at the previous value so the next tick retries. Don't move
  this checkpoint earlier.
- **New `[[repo]]` schema fields touch three places** in lockstep:
  `src/types.ts`, `src/config.ts` (zod schema + expansion), and the
  README config section. Missing any of those is a real defect.
- **Internal contracts are trusted.** Don't add null-checks, try/catch,
  or fallbacks at internal boundaries. Validation lives at system
  edges (config load, model output, subprocess invocation, network).
