...
## Project-specific concerns for dispatch

Read `AGENTS.md` for the full project surface; the items below are the
load-bearing security invariants worth restating.

- **Issue content is attacker-controlled.** Anyone with read access on a
  watched repo can file an issue. Treat title, body, labels, and comments
  as untrusted in every code path that consumes them — triage input,
  curator input, the security-flag detector, and any new sink. Untrusted
  content reaches `oteam pull github` and `oteam assign` downstream.
- **Triage and curator outputs are validated JSON.** Both call paths
  zod-validate the model's output before any side effect. New side
  effects driven by model output must do the same — no "best effort"
  parsing of model prose.
- **The security-flag path is load-bearing.** Issues flagged as
  `secret-leak`/`vuln-disclosure`/`pii`/`abuse` must NOT reach the
  vault (which is git-synced markdown that may sync to other operators)
  and must NOT get labels (don't tip off attackers via public labels).
  Any new code path that handles ingested content must respect the
  same gate.
- **The on-disk redaction (`security-inbox/`) is defence in depth, not
  a robust scrubber.** Don't approve new code paths that rely on it
  alone to keep secrets out of the vault.
- **Subprocess sites are sensitive.** Dispatch shells out to `gh`,
  `oteam`, `osascript`, and the `claude` binary (via the Agent SDK).
  Never compose subprocess arguments from issue content via shell-true
  / string concatenation. Argument-array forms only.
- **Autopilot `fire` tier delegates to `oteam assign`**, which in
  upstream audits has spawned Claude with `--dangerously-skip-permissions`.
  Any change to the curator/fire path widens that delegation; treat as
  high-scrutiny.
- **Secret env vars** (`ANTHROPIC_API_KEY`, `GITHUB_BOT_TOKEN` if ever
  used here) must not appear in logs, error messages, or subprocess
  argv visible to other processes.
