...
## Project-specific invariants for dispatch

Read `AGENTS.md` for the full project surface; the items below are the
load-bearing product invariants worth restating. Defending these is your
core job.

- **dispatch's job ends at the pull.** It ingests issues into the vault.
  Deeper triage, role assignment, and execution belong to `oteam`. Push
  back on changes that expand dispatch into oteam's territory.
- **Never auto-create vault projects.** The contract is: validate,
  refuse with a clear message, tell the operator what to run
  (`oteam project init <project> --vault <vault> --no-edit`). Do not
  reverse this even as a convenience.
- **First-run on a new repo seeds the cursor to *now* and ingests
  nothing historical.** This prevents floods. Backfill is opt-in via
  hand-edit of `cursors.json`.
- **One machine bootstrapped, others ready.** Multiple concurrent
  pollers against the same repos is not a supported configuration
  (would produce duplicate vault tickets and merge conflicts in the
  git-synced vault).
- **Triage and curator outputs are strict JSON.** Don't add "soft"
  fallbacks for malformed model output — log + skip is the contract.
- **Autopilot tiers are a trust spectrum:** `off | curate-only | fire`.
  Don't add intermediate tiers without a strong, named justification.
- **Security-flagged issues never reach the vault** and never get
  labels applied. They land in `~/.local/state/dispatch/security-inbox/`
  for manual operator triage. New code that handles flagged content
  must respect this contract.
- **CLI surface:** `dispatch poll`, `process`, `setup`, `state show`,
  `config validate`. New top-level commands need a discoverable home
  in `dispatch --help` and a Day-to-day or Debugging entry in the README.
- **Out of scope today:** Linux / systemd; webhook / push triggers
  (the 5-minute poll is intentional and removes the need for a public
  endpoint); sinks beyond oteam (Linear, Notion, raw markdown).
- **Configuration property:** every setting in `[defaults]` has a
  reasonable default. New settings preserve that.
